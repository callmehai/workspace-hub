...
| SCRUM-64 | **OTP đăng ký qua Firebase** (cột `Users.Phone`/`PhoneVerified` + migration, loại bỏ BE OTP sender, `/auth/verify-phone`, login chặn chưa verify; FE gọi Firebase Auth + màn OTP) | — | 63 | ✅ Done (nhánh `feat/SCRUM-64-register-otp-twilio` sửa sang Firebase) — Migration `AddUserPhoneOtp` **đã apply DB**. BE: Xóa `ISmsSender`, `OtpService`, Redis store cho OTP. Sử dụng thư viện `FirebaseAdmin` để gọi `VerifyIdTokenAsync`. Register tạo user `PhoneVerified=false` (trả `RegisterResult`, KHÔNG phát token); `POST /auth/verify-phone` (nhận FirebaseToken → verify → set PhoneVerified=true → đăng nhập); login chặn chưa verify → 403 `PHONE_NOT_VERIFIED`. FE: Register thêm field SĐT (E.164) → FE gọi Firebase để lấy và verify mã, sau đó gửi Firebase ID Token về BE `/verify-phone`. |
