...
| SCRUM-84 | **[BE+FE] Calendar hardening (all-day 502, attachment, Jira due date)** | Khánh | ✅ Done — Fix kéo cả ngày→có giờ (GET+UPDATE thay Patch); sync `driveAttachments`; gộp editor + query `occurredFrom`/`occurredTo`; gỡ `calendarType`; sync `fields.duedate` hiển thị deadline Jira trên lịch/panel; deep-link scheduled email từ lịch. Test patch event + `JiraItemMapperTests`. **Bổ sung:** Hoàn thiện đồng bộ nhắc nhở 2 chiều (Popup/Email), lặp lại sự kiện (RRULE), giao diện nhắc nhở giống Google Calendar (có timepicker cho đơn vị Ngày/Tuần, mặc định nhắc nhở 1 ngày trước) và xử lý ngắt kết nối cũ khi Connect lại.  **Reminder model:** mỗi row `EventReminders` = 1 `reminderType` (`GooglePopup`/`GoogleEmail`/`InApp`, cột DB `ReminderType`); UI dropdown từng dòng; processor chỉ bắn `InApp`. |
