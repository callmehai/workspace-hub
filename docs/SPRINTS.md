...
| SCRUM-64 | **OTP đăng ký qua Email (Resend)** (rename `PhoneVerified`→`EmailVerified` + drop `Phone` + migration, sender hệ thống Resend, `/auth/send-otp` + `/auth/verify-otp`, login chặn chưa verify; FE bỏ field SĐT + màn OTP theo email) | Lộc | 63 | 🔄 In Progress (nhánh `fix/login-ux`) — **Đổi hướng SMS/Firebase → Email/Resend** (xem CHANGELOG [2026-07-16]). Đã merge develop + gỡ sạch Firebase, giữ base SMS-OTP (Redis/hash/cooldown). **Còn implement:** `ISystemEmailSender` + `ResendEmailSender`/`LogEmailSender`; đổi `OtpService` kênh SMS→Email; migration rename cột (**sửa tay `RenameColumn`**); rate limit IP cho register/send-otp; verify domain Resend; bọc try/catch gửi OTP trong register. Register tạo user `EmailVerified=false` (trả `RegisterResult`, KHÔNG phát token); `POST /auth/verify-otp` (mã 6 số → set `EmailVerified=true` → đăng nhập); login chặn chưa verify → 403 `EMAIL_NOT_VERIFIED`. |
...
- 64 migration `EmailVerified` giữ default **true** cho user cũ (không phá login hiện có); chỉ user đăng ký mới sau migration mới phải verify. ⚠️ Rename phải sửa tay thành `RenameColumn` — EF tự sinh `Drop+Add` sẽ set user chưa-verify thành `true`.
