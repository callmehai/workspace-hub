...
| SCRUM-83 | **[BE+FE] Calendar Event CRUD + đính kèm Google Drive** — write-back create/patch event, `GoogleDrivePickerModal`, resolve `driveItemIds`→Google attachment | Huy | ✅ Done — `2cb8cdd`. |
| SCRUM-84 | **[BE+FE] Calendar hardening (all-day 502, attachment, Jira due date)** | Khánh | ✅ Done — Fix kéo cả ngày→có giờ (GET+UPDATE thay Patch); sync `driveAttachments`; gộp editor + query `occurredFrom`/`occurredTo`; gỡ `calendarType`; sync `fields.duedate` hiển thị deadline Jira trên lịch/panel; deep-link scheduled email từ lịch. Test patch event + `JiraItemMapperTests`. **Bổ sung:** Hoàn thiện đồng bộ nhắc nhở 2 chiều (Popup/Email), lặp lại sự kiện (RRULE), giao diện nhắc nhở giống Google Calendar (có timepicker cho đơn vị Ngày/Tuần, mặc định nhắc nhở 1 ngày trước) và xử lý ngắt kết nối cũ khi Connect lại. **Editor Google-style:** modal thêm/sửa cùng kích thước cố định, responsive hai cột, Guests riêng, lịch lặp tùy chỉnh (interval/ngày trong tuần/điểm kết thúc), đồng thời gửi `recurrence` đúng ở cả create/edit; dropdown đồng bộ design system và hàng thông báo tối ưu thao tác chọn loại/khoảng thời gian/giờ nhắc. **Reminder model:** mỗi row `EventReminders` = 1 `reminderType` (`GooglePopup`/`GoogleEmail`/`InApp`, cột DB `ReminderType`); UI dropdown từng dòng; processor chỉ bắn `InApp`. **Invitation/RSVP:** Google `sendUpdates=all` gửi email mời/cập nhật; WorkspaceHub tạo `CalendarInvitations` + notification cho user nội bộ, nhận Accepted/Tentative/Declined ngay trong app, hiển thị event đã nhận kể cả khi chưa connect GCal, reconcile bằng `iCalUID`; đồng bộ 3 quyền khách Modify/Invite/See guest list và chặn cả UI lẫn backend. **Guest email bugfix:** email đang gõ được chốt trước click Lưu; editor hỏi `Gửi/Không gửi` khi danh sách khách đổi và map `sendUpdates=all|none`; xóa khách cuối cùng dọn metadata local, modal edit ưu tiên attendee live từ Google. |
