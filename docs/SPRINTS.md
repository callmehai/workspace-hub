...
| SCRUM-69 | Tích hợp Google People API gợi ý contact khi soạn email: sync **Contact + OtherContact** vào DB; autocomplete To/Cc/Bcc **cả hai nguồn**; debounce, chip; fallback nhập tay khi API lỗi | Khánh | ✅ Done — migration `AddGoogleContacts`; optional scopes `contacts.readonly` + `contacts.other.readonly`; `IPeopleGateway`/`PeopleGateway` (list phân trang); sync best-effort trong `GmailSyncService`; OData suggest qua `GET /api/Contacts?connectionId=` *(thống nhất SCRUM-76, thay `EmailContactSuggestions`)*; FE `EmailChipsInput` + `contactsApi.suggestContacts`; wire `SendEmail` + `ScheduledEmails`. Test: `GmailSyncServiceTests`, `GoogleContactServiceQueryableTests`, `ODataInMemoryEndpointsTests`. |
| SCRUM-76 | **Google Contacts write-back 2 chiều + trang `/contacts`** — CRUD `Source=Contact` write-back People API; OData thống nhất `GET /api/Contacts` (SQL push-down) cho suggest + trang; commands `ContactCommandsController`; lazy 403 scope write. **Phụ thuộc SCRUM-69.** | Khánh | ✅ Done |
| SCRUM-77 | **Contact detail + Google profile (core)** — `MetadataJson` profile; `GET /api/contacts/{id}`; PATCH/POST `profile`; `participantEmail` trên items; FE `ContactDetailPanel` + recent emails + SendEmail prefill. Mở rộng SCRUM-76. | Khánh | ✅ Done |
