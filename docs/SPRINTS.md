...
| SCRUM-46 | FE: Write-back actions + xử lý 409 conflict | Vũ | ✅ Done — Hỗ trợ Email (star/read/label/trash, **kèm giao diện Thread View + Reply/Forward/đính kèm + preview/download Attachment + gộp thread ở list**), Event (CRUD), File (rename/trash), Ticket (assignee/priority/transition/comment) kèm ETag/version conflict resolution (Fix lỗi update assignee lần đầu tiên); Thêm Project filter và thông tin project vào Ticket. **Calendar view:** route `/calendar`, chỉ mở từ Tất cả mục / nguồn Google Calendar / folder (Email–Jira–Drive chỉ List/Board); xem Tháng/Tuần, hàng Cả ngày, slot tuần 30 phút, overlay Event + email hẹn giờ + deadline Jira, lọc 3 lớp, folder context; chỉ Event được kéo-thả và sửa/xoá tại lịch. **Ticket overhaul:** bỏ form "Sửa ticket" lớn → **sửa INLINE ngay tại từng field** (`JiraTicketPanel`): bút chì cho Summary/Description/Labels, dropdown cho Ưu tiên/Assignee(search)/Trạng thái(transition) — chọn/lưu là PATCH 2 chiều luôn. Thêm **comment CRUD** (list/add/edit/delete) + **attachment thật** (list/tải xuống/upload ≤25MB/xoá) 2 chiều Jira qua `IJiraTicketService` + endpoints `/api/items/{id}/comments|attachments`. |
...
| SCRUM-79 | **Google Drive: tạo folder + chia sẻ** — `POST /api/drive/folders`; permissions CRUD + link sharing (`/api/drive/items/{id}/permissions`, `link-sharing`); sync metadata `isFolder`/`parents`; FE dialog share + modal tạo folder + wire Integrations/Toolbar/ItemDetail | Dũng | ✅ Done — BE: `DriveController`, `DriveSharingService`, `IDriveGateway`/`DriveGateway`, DTO+validators, `DriveItemMapper` + `GoogleDriveGateway` fields parents; test `DriveSharingServiceTests` (5). FE: `types/drive.ts`, `driveApi.ts`, `DriveShareDialog`, `CreateDriveFolderModal`, i18n `drive.*`, wire B4; fix modal dropdown clip. Spec: `docs/DRIVE_FOLDER_SHARING.md`. |
| SCRUM-82 | **[FE] Calendar workspace view (Tháng / Tuần)** — route `/calendar`, grid Tháng+Tuần, drag event, overlay Scheduled/Jira, view switcher, modal editor cơ bản | Vũ | ✅ Done — `d3c501d`, `7a37694`, `a1ba25a`. |
| SCRUM-8x | **[BE+FE] Calendar Event CRUD + đính kèm Google Drive** — write-back create/patch event, `GoogleDrivePickerModal`, resolve `driveItemIds`→Google attachment. *Chưa tạo trên Jira — số tạm; code đã có (`2cb8cdd`).* | Huy | ✅ Done (code) — chờ tạo ticket Jira + gán key thật. |
| SCRUM-83 | **[BE+FE] Calendar write-back hardening** — all-day↔timed 502, sync attachments, gộp editor, bỏ `calendarType`/task | Khánh | ✅ Done — `38347dc`: GET+UPDATE thay Patch (không lẫn date/dateTime); gộp `GoogleCalendarGateway`→`CalendarGateway`; sync cancelled→hard-delete; map Drive attachments; `occurredFrom`/`occurredTo` overlap. `1b41fc8`: gỡ app-only task/`calendarType`. `e246cb3`: bỏ full-page reload sau CRUD event. FE: DatePicker/TimePicker + `CalendarEventEditorModal` duy nhất; xoá `CreateEventModal`. Test: `ItemWriteBackServicePatchEventTests`. Docs API/CHANGELOG. *(Trước docs gộp nhầm reminder/invitation/RRULE vào 83.)* |
| SCRUM-85 | **[FE(+test)] Calendar — UI khách mời: cập nhật danh sách + hỏi gửi email Google + RSVP** | Khánh | ✅ Done — **Hoàn thiện / fix trên nền đã có** (invitation + guest permissions: Vũ; reminder/RSVP nền: Huy). Khánh: redesign guest input/list + dialog hỏi Gửi/Không gửi khi đổi khách (`a97fd97`); đồng bộ popup sau add/remove; RSVP/conflict copy FE (`ccf75e9`); test `CalendarGatewayAttendeeUpdateTests`. |
| SCRUM-86 | **[BE+FE] Calendar — Due date Jira trên lịch + nhắc nhở 3 loại + popup chi tiết sự kiện** | Khánh | ✅ Done — **Hoàn thiện / fix trên nền reminder–RSVP của Huy.** Due date: sync + hiển thị due Jira đúng ngày trên lịch/panel (`b5c6155`). Reminder 3 kênh (`GooglePopup`/`GoogleEmail`/`InApp`) + migration; processor InApp; thay modal → `EventDetailPopup` + màu entry (`57cf501`). Docs API/DATABASE. |
| SCRUM-84 | **[BE+FE] Fix reconnect sau lỗi sync — OAuth callback báo trùng connection** | Khánh | ✅ Done — `CompleteConnectionAsync` upsert: `GetByUniqueKey` → nếu đã có thì `DisconnectAsync` rồi tạo connection mới (`Status=Active`), không 409. FE «Kết nối lại» gọi cùng OAuth start. **Bổ sung UX:** sync tay (`POST .../sync`, `markProviderError`) ghi `Status=Error` khi provider 502; Integrations/Toolbar `invalidateQueries(['connections'])` khi sync fail → badge lỗi hiện ngay không cần F5. |
