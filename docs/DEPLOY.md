...
`SITE_ADDRESS`, `MSSQL_SA_PASSWORD`, `JWT_SECRET`, `CRON_SECRET`, `GOOGLE_CLIENT_ID/SECRET`, `Email__Resend__ApiKey` + `Email__Resend__FromAddress` (SCRUM-64 — OTP email, thiếu → dev log OTP), `R2_*` (SCRUM-75 — avatar, xem dưới).
...
- OTP đăng ký (SCRUM-64) gửi qua email bằng **Resend**: set `Email__Resend__ApiKey` + `Email__Resend__FromAddress` (domain đã verify trên Resend). Thiếu → dev fallback ghi OTP ra log (không gửi mail thật). **Verify domain `workspace-hub.space`** (SPF/DKIM) để gửi được tới email bất kỳ; test mode chỉ gửi tới chủ tài khoản Resend.
