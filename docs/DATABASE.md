...
- **GoogleContacts** (✅ SCRUM-69, write-back SCRUM-76, profile SCRUM-77) — cache contact Google theo Connection Gmail (gợi ý To/Cc/Bcc + trang `/contacts`). Cột: `ConnectionId` FK→Connections **CASCADE**, `Email` (nvarchar 320 nullable, **primary email** denormalized từ `MetadataJson`), `DisplayName`, `Source` enum string (`Contact` / `OtherContact`), `ExternalResourceName`, `Etag` (nvarchar 128), `MetadataJson` (nvarchar max, JSON camelCase profile), `SyncedAt` UTC, `UpdatedAt` UTC nullable. **UNIQUE(ConnectionId, ExternalResourceName)** khi resourceName không null; index thường `(ConnectionId, Email)`. **1 row / Google person** (`ExternalResourceName`). Person không email vẫn cache (chỉ tên/SĐT). Orphan xóa khi Google không còn trả person. Sync merge giữ `UpdatedAt` nếu write-back trong ~30s. Write-back: `ApplyDetailToResource` upsert 1 row theo resourceName.
