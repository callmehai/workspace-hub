...
- **GoogleContacts** (✅ SCRUM-69, write-back SCRUM-76) — cache contact Google theo Connection Gmail (gợi ý To/Cc/Bcc + trang `/contacts`). Cột: `ConnectionId` FK→Connections **CASCADE**, `Email` (nvarchar 320, lưu lower-case), `DisplayName`, `Source` enum string (`Contact` / `OtherContact`), `ExternalResourceName`, `Etag` (nvarchar 128, version token People API), `SyncedAt` UTC (lần pull sync gần nhất), `UpdatedAt` UTC nullable (lần write-back thành công). UNIQUE(ConnectionId, Email). Sync **kèm mỗi lần sync Gmail** (`connections.list` + `otherContacts.list`, lưu etag); full replace mỗi lần sync. Write-back CRUD chỉ `Source=Contact` — upsert 1 row sau mutate, không full replace ngay. Nguồn kích sync: cron định kỳ (SCRUM-72, ~60s), `POST /api/connections/{id}/sync`, hoặc lazy khi GET items.
