...
- `POST /api/drive/folders/upload` — [Authorize]. `multipart/form-data`: `connectionId`, `files` (IFormFile[]), `paths` (string[], webkitRelativePath — số phần tử khớp `files`), `parentItemId?`. Tạo cây folder trên Drive rồi upload từng file; mỗi folder/file thành công được `SaveChanges` ngay (lỗi giữa chừng vẫn giữ phần đã lên). Giới hạn **200 file**, **100 MB**/file, **500 MB** tổng (`[RequestSizeLimit]`). Trả **200** `{ items: ItemResponse[], filesUploaded, foldersCreated }`. (400 thiếu file / paths không khớp / file rỗng / vượt giới hạn, 404, 422 parent không hợp lệ / connection không Drive-Active, 403, 502)
