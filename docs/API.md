...
- `POST /api/connections/oauth/start` — `{integrationKey, serviceType, redirectUri}` → `{authorizationUrl, state}`. Scope = full của service đó (dev quyết). Mỗi lần chỉ connect 1 service. **Multi-account (chỉ Google):** authorization URL Google gắn `prompt=select_account consent` (cố ý) → user chọn được tài khoản KHÁC ⟹ 1 user connect được nhiều Gmail/Calendar/Drive (khác email). **Jira giữ `prompt=consent`** (không dùng `select_account` — không mở UI multi-account kiểu Google). (404 integration không tồn tại, 422 serviceType không hợp lệ / provider không hỗ trợ serviceType / integration disabled)
- `POST /api/connections/oauth/callback` — `{code, state}` → 201 tạo Connection row cho **tài khoản** vừa chọn. Response: `{integrationKey, providerAccountId, connections: [{id, serviceType, status}]}`. Tài khoản mới (khác `providerAccountId`) → thêm row mới; **trùng ĐÚNG service+account đã có → UPSERT** (cập nhật token + Status=Active — chính là luồng "Kết nối lại"; trước 2026-07-18 trả 409 làm nút Reconnect vô dụng). (400 CSRF, 400 provider từ chối/scope thiếu)
...
- `PATCH /api/items/{id}` ⭐ — write-back, body theo Type:
  ...
  - Event: `{title?, start?, end?, allDay?, location?, attendees?[], description?, driveItemIds?[], reminders?[], recurrence?[], guestsCanModify?, guestsCanInviteOthers?, guestsCanSeeOtherGuests?, sendUpdates?}` — sau patch cập nhật `occurredAt`/`dueAt` + metadata. `sendUpdates=true|false` map sang Google `all|none` khi request có attendees (mặc định `true`); FE hỏi người dùng khi thêm/xóa khách. Khách chỉ sửa khi `guestsCanModify`; chỉ đổi attendees khi `guestsCanInviteOthers`; chỉ organizer đổi ba quyền. `allDay=true` → Google `EventDateTime.Date`. Đổi all-day ↔ timed dùng GET+UPDATE (không Patch) để tránh lỗi `Invalid start time`. Khi xóa khách cuối cùng, metadata local xóa hẳn khóa `attendees` thay vì giữ snapshot cũ.
  - File: `{name?, isTrashed?}`
...
- `GET /api/calendarinvitations?from=&to=` — lời mời của user hiện tại trong khoảng lịch; gồm snapshot event, trạng thái `NeedsAction|Accepted|Tentative|Declined`, `inviteeItemId`, `googleSyncPending` và ba quyền khách.
- `GET /api/calendarinvitations/{id}` — chi tiết một lời mời của user hiện tại (dùng deep-link notification `/calendar?invitation={id}`).
- `POST /api/calendarinvitations/{id}/respond` — body `{response:"Accepted"|"Tentative"|"Declined", comment?}`. Lưu RSVP trong app; nếu có GCal connection thì tìm event bằng `iCalUID` và ghi ngay lên Google, nếu chưa có thì `googleSyncPending=true` để vẫn hiển thị đúng trong WorkspaceHub và reconcile ở lần sync sau.
- In-app notification chỉ tạo khi email khách trùng một user WorkspaceHub đang active. Khách ngoài hệ thống chỉ nhận email Calendar từ Google.
...
- `POST /api/drive/files` — [Authorize]. `multipart/form-data`: `connectionId` (Guid), `file` (IFormFile), `parentItemId?` (Guid). Upload một file từ máy lên Google Drive + tạo Item local ngay. Giới hạn **100 MB**/file (`[RequestSizeLimit]`). Trả **201** `ItemResponse`. (400 thiếu file / file rỗng, 404 connection/parent, 422 vượt 100 MB / parent không phải folder Drive / khác connection / parent đã trash / connection không Active, 403 thiếu scope Drive, 502 provider)
- `POST /api/drive/folders/upload` — [Authorize]. `multipart/form-data`: `connectionId`, `files` (IFormFile[]), `paths` (string[], webkitRelativePath — số phần tử khớp `files`), `parentItemId?`. Tạo cây folder trên Drive rồi upload từng file; mỗi folder/file thành công được `SaveChanges` ngay. Giới hạn **200 file**, **100 MB**/file, **500 MB** tổng (`[RequestSizeLimit]` + `[RequestFormLimits]` để không vướng `MultipartBodyLengthLimit` mặc định 128 MB). **KHÔNG atomic** — 1 file lỗi giữa chừng chỉ vào `failed`, các file còn lại vẫn upload. Trả **200** `{ items: ItemResponse[], filesUploaded, foldersCreated, failed: { relativePath, fileName, error }[] }` (`failed` rỗng nếu trọn vẹn; FE hiển thị "đã lên X/Y file"). (400 thiếu file / paths không khớp / file rỗng / vượt giới hạn, 404, 422 parent không hợp lệ / connection không Drive-Active, 403, 502)
- `GET /api/drive/items/{itemId}/content` — [Authorize]. **Tải/xem nội dung file** (BE proxy stream từ Google, KHÔNG buffer toàn file vào RAM). Query `?dl=true` → `Content-Disposition: attachment` (tải xuống); mặc định `inline` (dùng cho preview ảnh qua blob). Google-native docs (Docs/Sheets/Slides/Drawing) được **export** sang PDF/PNG. Trả **200** stream. (404 item/connection, 422 item là folder / không phải File Drive / connection không Drive-Active / Google-native không export được, 403 thiếu quyền tải, 502 provider). FE dùng cookie auth (`<a download>` gửi cookie; ảnh preview lấy blob qua axios để hưởng refresh 401).
- `GET /api/drive/items/{itemId}/thumbnail` — [Authorize]. Ảnh **thumbnail để preview** (proxy `thumbnailLink` của Google bằng bearer token). Trả **200** ảnh, hoặc **204** nếu file không có thumbnail (folder / Google chưa render). Header `Cache-Control: private, max-age=300`. (404, 422, 502)
...
- `PUT /api/drive/items/{itemId}/link-sharing` — [Authorize]. Body `{ enabled, role?, confirmRestrictParent? }`. `enabled=true` → bật anyone-with-link (`role` bắt buộc: reader|commenter|writer); `enabled=false` → tắt link. **Case 1 (giống Google Drive):** tắt link file khi folder mẹ đang anyone → **409** + body `DriveLinkRestrictConflict` (service ném `ConflictException` kèm Payload; middleware ghi payload làm body — không envelope `{error,message}`). Trừ khi `confirmRestrictParent: true` → tắt link **cả file lẫn folder mẹ**. Case 2 (folder private, file public) không hỏi — bật thẳng. Trả **200** `DrivePermissionDto` hoặc `null` khi tắt. (400 validation, 404, 409 conflict Case 1, 502)
...
**FE (SCRUM-79):** `DriveShareDialog`, `DriveLinkRestrictDialog` (Case 1), `CreateDriveFolderModal`, `DriveNewMenu` (upload file/folder), `driveApi`; entry: ItemDetail (Chia sẻ + folder con), Integrations (Tạo folder), WorkspaceToolbar (Folder Drive / New). i18n `drive.*`.
