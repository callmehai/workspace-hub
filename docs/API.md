...
## Google Contacts — cache + write-back (SCRUM-76) ⭐
- `GET /api/Contacts?connectionId=` — [Authorize]. **OData ⊕** convention route (`ContactsController`): `$filter` (vd `contains(Email,'al')`, `Source eq 'Contact'`), `$orderby`, `$top`, `$skip`, `$count`, `$select`. Query OData **PascalCase** tên property CLR; JSON camelCase. Bắt buộc `connectionId` (scope server-side theo connection Gmail của user). Trả `{ value: [{ id, connectionId, email, displayName, source, etag, syncedAt, updatedAt }], @odata.count? }`. **SQL push-down** — `IQueryable` EF projection `AsNoTracking`. Dùng cho **cả** gợi ý To/Cc/Bcc (`$select=email,displayName,source`, `$top=10`) **và** trang `/contacts`. 404 connection, 422 không phải Gmail / không Active. Scope đọc optional `contacts.readonly` + `contacts.other.readonly` — thiếu → sync rỗng; scope ghi `contacts` optional — lazy 403 khi mutate.
- `POST /api/contacts` — [Authorize]. Body `{ connectionId, email, displayName? }`. Tạo trên Google rồi upsert cache (`Source=Contact`). Trả `201 ContactDto`. (403 thiếu scope write, 409 trùng email)
- `PATCH /api/contacts/{id}` — [Authorize]. Body `{ displayName?, email?, etag }` — **etag bắt buộc**. Chỉ `Source=Contact`. Conflict etag → 409. Trả `200 ContactDto`.
- `DELETE /api/contacts/{id}` — [Authorize]. Xoá Google + cache. Chỉ `Source=Contact`. Trả `204`.
- **Breaking:** `GET /api/EmailContactSuggestions` đã gộp vào `GET /api/Contacts`. REST list envelope `GET /api/contacts` đã bỏ — dùng OData.
