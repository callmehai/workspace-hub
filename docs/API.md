...
- `POST /api/connections/oauth/start` — `{integrationKey, serviceType, redirectUri}` → `{authorizationUrl, state}`. Scope = full của service đó (dev quyết). Mỗi lần chỉ connect 1 service. **Multi-account:** authorization URL Google/Atlassian gắn `prompt=select_account consent` (cố ý) → user chọn được tài khoản KHÁC ⟹ 1 user connect được nhiều Gmail/Calendar/Drive (khác email). (404 integration không tồn tại, 422 serviceType không hợp lệ / provider không hỗ trợ serviceType / integration disabled)
- `POST /api/connections/oauth/callback` — `{code, state}` → 201 tạo Connection row cho **tài khoản** vừa chọn. Response: `{integrationKey, providerAccountId, connections: [{id, serviceType, status}]}`. Tài khoản mới (khác `providerAccountId`) → thêm row mới; **409 chỉ khi trùng ĐÚNG service+account đã có** (unique `(UserId,Provider,ServiceType,ProviderAccountId)`). (400 CSRF, 400 provider từ chối/scope thiếu, 409 trùng service+account)
