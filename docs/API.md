...
- `POST /api/internal/process-sync` — header `X-Cron-Secret` (cùng `Cron:Secret` với `process-scheduled`). Không JWT. Quét mọi Connection Active + Integration enabled → debounce → refresh token nếu cần → `ConnectionSyncDispatcher.SyncAsync` (Gmail/GCal/Drive/Jira). Mỗi connection lỗi → `Status=Error`, batch vẫn chạy tiếp. Trả `200 ProcessSyncResult { totalConnections, successCount, skippedCount, errorCount, details? }`. ✅ SCRUM-72.
  - **Cron ngoài** gọi định kỳ (khuyến nghị 5 phút/lần) — thiết kế prod mặc định (`Cron:SyncAutoRun=false`).
  - **Auto-cron nội bộ (tuỳ chọn dev):** `Cron:SyncAutoRun=true` → `ConnectionSyncProcessorService` gọi service mỗi `Cron:SyncIntervalSeconds` (dev 60s, prod default 300s). Không thay endpoint HTTP — prod vẫn dùng cron ngoài.
