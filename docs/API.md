...
- `PATCH /api/items/{id}` ⭐ — write-back, body theo Type:
  ...
  - Event: `{title?, start?, end?, allDay?, location?, attendees?[], description?, calendarType?, driveItemIds?[]}` — sau patch cập nhật `occurredAt`/`dueAt` + `metadata.start`/`end`/`allDay`. `allDay=true` → Google `EventDateTime.Date`.
  - File: `{name?, isTrashed?}`
