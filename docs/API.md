...
- `GET /api/emails/{itemId}/thread` — [Authorize]. Lấy **live** toàn bộ luồng hội thoại (thread) của email từ Gmail (`threads.get`, không lưu cứng DB). Trả `200 { threadId, subject, messages[] }` với mỗi message `{ messageId, from, to[], cc[], bcc[], subject, bodyHtml, bodyPlainText, occurredAt, isUnread, isStarred, hasAttachment, attachments[] }`; `attachments[]` = `{ attachmentId, filename, mimeType, size }`. (404 item / message không có threadId).
- `POST /api/emails/reply` — [Authorize]. Body `{ connectionId, itemId, bodyHtml, replyAll, cc[]?, bcc[]?, attachments[]? }`. Reply / Reply-All trong thread có sẵn (header `In-Reply-To`/`References`; Reply-All tự lấy cc live nếu metadata cũ thiếu, loại email của chính mình). `attachments[]` như `send`. Trả `200 { messageId, threadId, sentAt }`.
- `POST /api/emails/forward` — [Authorize]. Body `{ connectionId, itemId, to[], cc[]?, bcc[]?, bodyHtml, includeAttachments, attachments[]? }`. Forward: quote nội dung gốc, `includeAttachments=true` đính lại file gốc (tải từ Gmail), `attachments[]` = file user thêm mới. Trả `200 { messageId, threadId, sentAt }`.
- `GET /api/emails/{itemId}/attachments/{attachmentId}` — [Authorize]. Stream 1 file đính kèm (binary) từ Gmail qua backend gateway (không lộ token client). Trả file (`Content-Disposition: attachment`). (404 attachment / message).
- `GET /api/emails/{itemId}/messages/{messageId}/attachments/zip` — [Authorize]. Tải **toàn bộ** attachment của 1 message trong thread, đóng gói `.zip` (server-side `ZipArchive`, tên trùng tự thêm hậu tố). Trả `application/zip` (`attachments.zip`). (422 message không có attachment).
...
- `POST /api/emails/reply` — [Authorize]. Trả lời (Reply / Reply-All) trong luồng hội thoại email đã có. Body `{ connectionId, itemId, cc?[], bcc?[], bodyHtml, replyAll }`. Reply-All tự động lấy To/Cc từ metadata + live Gmail API, loại trừ email của chính user (exact match, không substring). Sử dụng header `In-Reply-To`/`References` chuẩn RFC 5322 (lấy `Message-ID` gốc từ metadata) để mail client ngoài Gmail cũng nhóm thread đúng. Trả `200 { messageId, threadId, sentAt }`. (400 validation, 404 item/connection, 422 item không có threadId/connection không Gmail, 502 provider lỗi)
- `POST /api/emails/forward` — [Authorize]. Chuyển tiếp email trong luồng. Body `{ connectionId, itemId, to[], cc?[], bcc?[], bodyHtml, includeAttachments }`. Khi `includeAttachments=true`, tải attachment gốc từ Gmail và đính kèm vào thư mới (MIME multipart/mixed). Trả `200 { messageId, threadId, sentAt }`. (400 validation, 404 item/connection, 422 item không có threadId, 502 provider lỗi)
- `GET /api/emails/{itemId}/attachments/{attachmentId}` — [Authorize]. Tải file đính kèm trực tiếp từ Gmail qua Backend Gateway (stream binary). Trả `200` với `Content-Type` tương ứng + `Content-Disposition: attachment`. Ownership check: attachment phải thuộc message thuộc item thuộc connection của user. (404 item/attachment không tồn tại, 422 item không có threadId, 502 provider lỗi)
