...
| `GET /api/Contacts` | ⊕ | Email, DisplayName, Source (contains) | DisplayName, Email, SyncedAt, UpdatedAt |
...
- `GET /api/items?folderId&statuses&types&isImportant&tagIds&projectKey&assignee&gmailLabel&connectionId&participantEmail&search&page&limit` — envelope. Trả kèm ETag. `statuses`/`types`/`tagIds` **đa chọn** (query lặp key, vd `?statuses=Inbox&statuses=Doing&types=Email`) — không truyền = không lọc field đó (FE: chip toggle kiểu tag, bấm lại để bỏ). `tagIds` ✅ **SCRUM-71** = lọc item gắn **bất kỳ tag nào** trong danh sách (OR; join `TagAssignment`; lặp key `?tagIds={g1}&tagIds={g2}`). `projectKey` = lọc theo dự án (Jira Ticket). `assignee` = lọc Ticket theo **người phụ trách** (accountId; `"unassigned"` = chưa gán) — match `metadata.assigneeAccountId`. `connectionId` = lọc item thuộc **một connection** (Drive modal chọn folder cha, v.v.). `gmailLabel` = lọc email theo **Gmail label** (`INBOX`/`SENT`/`DRAFT`/`STARRED`/`IMPORTANT`/`CATEGORY_PROMOTIONS`/`CATEGORY_SOCIAL`/`CATEGORY_UPDATES`/...) — match token trong `metadata.labels`; chỉ Email có labels nên loại khác tự loại. `participantEmail` ✅ **SCRUM-77** = lọc thread **Email** có địa chỉ trong `metadata.from` / `to` / `cc` / `bcc` (match `"email@domain"` có dấu ngoặc kép, lowercase); chỉ truyền `participantEmail` → mặc định `types=Email`. **SPAM/TRASH chưa lọc được** (sync bỏ qua, `includeSpamTrash=false`); Purchases/Bills của Gmail là view ML nội bộ, **không** expose qua API. Mỗi item trong response trả kèm `tags: [{id, name, color}]` (tag đang gắn). **OData ⊕** (target — $filter/$orderby/$select/$top/$skip/$count thay query param thủ công; vẫn scope theo CurrentUserId trước).
...
## Google Contacts — cache + write-back (SCRUM-76/77) ⭐
- `GET /api/Contacts?connectionId=` — [Authorize]. **OData ⊕** — list **1 row / Google person**. `email` = primary (nullable). `$filter` `contains(Email,...)` / `contains(DisplayName,...)`. Trang `/contacts`.
- `GET /api/contacts/suggest?connectionId=&query=&limit=` — [Authorize]. Gợi ý To/Cc/Bcc: flatten **mọi email** trong `MetadataJson` của từng person. `query` ≥2 ký tự. Trả `[{ email, displayName, source }]`.
- `GET /api/contacts/{id}` — [Authorize]. ✅ **SCRUM-77**. Trả `ContactDetailDto` từ **cache DB** (`MetadataJson` + `etag` đã sync). `OtherContact` → `readOnly: true`. Không gọi People API khi đọc (cùng pattern email/Drive: sync → đọc cache, mutate mới gọi provider). 404 owner/connection.
- `POST /api/contacts` — [Authorize]. Body `{ connectionId, email, displayName? }` hoặc `{ connectionId, email, profile? }`. Tạo trên Google rồi upsert **1 row cache** (`Source=Contact`). Trả `201 ContactDto`. (403 thiếu scope write, 409 trùng primary email)
- `PATCH /api/contacts/{id}` — [Authorize]. Body `{ etag, displayName?, email?, profile? }` — **etag bắt buộc**; `profile` phải có ≥1 email. Chỉ `Source=Contact`. Conflict etag → 409. Trả `200 ContactDetailDto`. Sau write-back upsert 1 row theo `ExternalResourceName`.
- `DELETE /api/contacts/{id}` — [Authorize]. Xoá Google + row cache theo `resourceName`. Chỉ `Source=Contact`. Trả `204`.
- **Breaking:** `GET /api/EmailContactSuggestions` đã gộp; suggest autocomplete dùng `GET /api/contacts/suggest` (không OData).
