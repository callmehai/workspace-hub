...
export function FriendMultiSelect({ friends, value, onChange, disabled, className = '' }: FriendMultiSelectProps) {
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      const flipUp = r.bottom + menuH + 8 > window.innerHeight && r.top - menuH - 8 > 0;
      setPos({ top: flipUp ? r.top - menuH - 6 : r.bottom + 6, left: r.left, width: r.width });
    };
    place();
    ...
    <div ref={ref} className="relative">
      ...
      {open && createPortal(
        <div
          ...
          className="z-[200] bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden"
        >
          {/* Ô tìm kiếm */}
