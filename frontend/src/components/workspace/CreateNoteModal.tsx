...
export const CreateNoteModal = ({ isOpen, onClose, folder }: Props) => {
  ...
  const createNote = useMutation({
    ...
    onSuccess: (created) => {
      markSeen(created.id); // note tự tạo = đã xem, không hiện "Chưa xem"
      queryClient.invalidateQueries({ queryKey: ['items'] });
      queryClient.invalidateQueries({ queryKey: ['folders'] });
      setForm({ title: '', contentMarkdown: '' });
      onClose();
      toast.success(t('createNote.created'));
    },
    onError: (err) => handleApiError(err, t('createNote.createFail'), { navigate }),
  });
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-slate-900 rounded-xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col">
          ...
          <h2 className="text-[16px] font-semibold text-slate-900 dark:text-slate-100">
            ...
            {folder && (
              <span className="ml-2 text-[12px] font-medium text-slate-500 dark:text-slate-400">
                {t('createNote.toFolder', { name: folder.name })}
              </span>
            )}
          </h2>
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-[13px] font-medium text-slate-700 dark:text-slate-200 mb-1.5">{t('sendEmail.subject')}</label>
            <input
              type="text"
              value={form.title}
          ...
          <div>
            <label className="block text-[13px] font-medium text-slate-700 dark:text-slate-200 mb-1.5">{t('createNote.contentLabel')}</label>
            <textarea
        ...
        <div className="px-5 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex justify-end gap-2">
          ...
          >
            ...
            {createNote.isPending ? t('createEvent.creating') : t('createNote.create')}
          </button>
        </div>
      </div>
    </div>
