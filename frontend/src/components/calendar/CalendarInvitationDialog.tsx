          ...
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-300">
                {lang === 'vi' ? 'Lời mời tham gia lịch' : 'Calendar invitation'}
              </p>
              <h2 className="mt-1 break-words text-xl font-bold text-slate-900 dark:text-white">{invitation.title}</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {lang === 'vi' ? 'Người tổ chức' : 'Organizer'}: {invitation.organizerName} · {invitation.organizerEmail}
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 space-y-3 text-sm text-slate-600 dark:text-slate-300">
          ...
          {invitation.googleSyncPending && invitation.status !== 'NeedsAction' && (
            <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
              {lang === 'vi' ? 'Phản hồi đã lưu trong WorkspaceHub và đang chờ đồng bộ Google Calendar.' : 'Saved in WorkspaceHub and waiting for Google Calendar sync.'}
            </p>
          )}
        </div>
