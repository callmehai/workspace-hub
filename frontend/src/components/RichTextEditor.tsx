...
export function RichTextEditor({ value, onChange, placeholder, className }: RichTextEditorProps) {
  ...
  const btnClass = 'w-8 h-8 flex items-center justify-center rounded-md text-gray-600 hover:bg-gray-200 transition-colors';
  const disCls = 'disabled:opacity-40 disabled:pointer-events-none';

  const editor = (
    <div className={`border border-gray-300 rounded-lg overflow-hidden bg-white flex flex-col focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-colors ${expanded ? 'h-full' : (className ?? '')}`}>
      ...
      <div className="flex items-center flex-wrap gap-0.5 px-2 py-1.5 border-b border-gray-200 bg-gray-50 shrink-0">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 h-8 rounded-md text-xs font-medium transition-colors ${htmlMode ? 'bg-brand-600 text-white' : 'text-gray-600 hover:bg-gray-200'}`}
        >
      ...
      <div className={`relative flex flex-col ${expanded ? 'flex-1 min-h-0' : ''}`}>
        {htmlMode ? (
          <textarea
            ...
            className={`${expanded ? 'flex-1 resize-none' : 'min-h-[300px] resize-y'} w-full px-3 py-2.5 text-sm font-mono text-gray-800 outline-none leading-relaxed`}
          />
        ) : (
          <div className={`relative ${expanded ? 'flex-1 min-h-0' : ''}`}>
            <div
              ...
              className={`wh-rte ${expanded ? 'h-full overflow-y-auto' : 'min-h-[300px]'} px-3 py-2.5 text-sm text-gray-800 outline-none leading-relaxed`}
            />
            {!value && (
      ...
      <div className="flex items-center justify-end px-2 py-1 border-t border-gray-200 bg-gray-50 shrink-0">
        <button
          type="button"
          title={expanded ? 'Thu nhỏ (Esc)' : 'Phóng to'}
          onClick={() => setExpanded((e) => !e)}
          className="flex items-center gap-1.5 px-2 h-7 rounded-md text-xs font-medium text-gray-500 hover:bg-gray-200 hover:text-gray-700 transition-colors"
        >
