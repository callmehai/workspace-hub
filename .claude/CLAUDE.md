...
- Migration mới mỗi thay đổi schema, KHÔNG sửa migration đã commit. Hiện có **13** (thứ tự): `InitialCreate`, `UsersMultiAuth`, `ModelBConnections`, `RemoveClientCredentialsFromIntegration`, `AddAtlassianIntegrationSeed`, `AddCreatedAtToScheduledEmails`, `AddUserPhoneOtp`, `AddTagUserNameUniqueIndex` (SCRUM-70 — unique `Tags(UserId,Name)`), `EnableJiraIntegration` (bật `atlassian` IsEnabled=true), `AddGoogleContacts`, `AddItemThreadId`, `AddScheduledEmailAttachments`, `AddFriendSystem`. **SCRUM-64 (đang làm) sẽ thêm migration thứ 14** rename `PhoneVerified`→`EmailVerified` + drop `Phone` (⚠️ sửa tay thành `RenameColumn`). Prod tự chạy migration khi deploy (`Db__AutoMigrate=true`); local phải `dotnet ef database update` tay.
...
- ✅ **Done (auth overhaul):** 62 HttpOnly cookie + CSRF, 63 refresh token + Redis rotation. **64 OTP đang 🔄 đổi hướng SMS/Firebase → Email/Resend** (nhánh `fix/login-ux`; xem CHANGELOG [2026-07-16]).
